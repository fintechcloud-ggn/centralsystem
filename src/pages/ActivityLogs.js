      ...
      <section className="mx-auto w-full max-w-3xl">
        <div className="rounded-[28px] border border-white/80 bg-white/80 p-7 text-center shadow-[0_18px_50px_rgba(148,163,184,0.12)]">
          ...
          <p className="mt-2 text-sm text-slate-500">
            Only super user can view activity logs.
          </p>
        </div>
      </section>
    );
    ...
    <section className="mx-auto w-full max-w-6xl space-y-5 2xl:max-w-[90rem]">
      <div className="rounded-[28px] border border-white/80 bg-gradient-to-r from-[#f7f6fd] via-[#f8f5fb] to-[#efe5ff] p-5 shadow-[0_18px_50px_rgba(148,163,184,0.12)] md:p-7">
        ...
        <p className="mt-1 text-sm text-slate-500">
          Super user actions are recorded here with admin identity and timestamp.
        </p>
        <input
          type="text"
          value={searchTerm}
          onChange={(event) => setSearchTerm(event.target.value)}
          placeholder="Search by admin, action, entity, details"
            ...
            {paginatedLogs.length > 0 ? (
              paginatedLogs.map((log) => (
                <tr key={log.id} className="border-t border-[#efedf8]">
                  ...
                  <td className="px-4 py-3 text-slate-700">{log.admin_role || "-"}</td>
                  <td className="px-4 py-3 text-slate-700">{formatLogAction(log.action)}</td>
